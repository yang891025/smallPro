Component({
  ...
  ready() {
    ...
    for (let i = 0; i < d.columns - 1; i++) {
      pickData.push(lmap[pickData[i][d.dataValue[i]].value])
    }

    // 初始化显示和选项数组
    let text = '';
    let multiIndex = d.dataValue;
    for (let i = 0; i < d.columns; i++) {
      text += pickData[i][d.dataValue[i]].name + ' '
      //multiIndex.push(0)
    }
  ...
  methods: {
    bindMultiPickerChange: function(e) {
      ...
      for (let i = 0; i < this.data.columns; i++) {
        ...
        value = this.data.multiArray[i][this.data.multiIndex[i]].value
      }
